    ...
    switch ($action) {
        case 'save':
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $stmt = $pdo->prepare("
                    UPDATE ssaa_feedback 
                    SET rating = ?, comments = ?, is_approved = 1
                    WHERE user_id = ? AND DATE(created_at) = CURDATE()
                ");
                $stmt->execute([$rating, $comments, $user_id]);
            } else {
                // Insert new feedback (auto-approved)
                $stmt = $pdo->prepare("
                    INSERT INTO ssaa_feedback (user_id, rating, comments, is_approved, created_at)
                    VALUES (?, ?, ?, 1, NOW())
                ");
        ...
        case 'get_all':
            // Get all SSAA feedback
            $pdo = get_db();
            
            $stmt = $pdo->prepare("
                SELECT sf.*, u.full_name, u.email
                FROM ssaa_feedback sf
        ...
        case 'get_stats':
            // Get SSAA feedback statistics
            $pdo = get_db();
            
            $stmt = $pdo->prepare("SELECT AVG(rating) as avg_rating FROM ssaa_feedback WHERE is_approved = 1");
            $stmt->execute();
            $avgRating = $stmt->fetch(PDO::FETCH_ASSOC);
            
            $stmt = $pdo->prepare("SELECT COUNT(*) as total_feedback FROM ssaa_feedback WHERE is_approved = 1");
            $stmt->execute();
