...
if (isset($_POST['export_report'])) {
    ...
    if ($exportFormat === 'excel') {
        ...
        if ($reportType === 'overview') {
            echo "Metric,Value\r\n";
            echo "Total Patients," . $analytics['total_patients'] . "\r\n";
            echo "Monthly Visits," . $analytics['monthly_visits'] . "\r\n";
            echo "Today's Visits," . $analytics['today_visits'] . "\r\n";
            echo "Weekly Visits," . $analytics['week_visits'] . "\r\n";
            echo "Average Rating," . $analytics['avg_rating'] . "\r\n";
            echo "Total Medicines," . $analytics['total_medicines'] . "\r\n";
            echo "Low Stock Alerts," . $analytics['low_stock_alerts'] . "\r\n";
            echo "Expired Medicines," . $analytics['expired_medicines'] . "\r\n";
            echo "Pending Clearances," . $analytics['pending_clearances'] . "\r\n";
            echo "Approved Clearances (Monthly)," . $analytics['approved_clearances'] . "\r\n";
            echo "Rejected Clearances (Monthly)," . $analytics['rejected_clearances'] . "\r\n";
            echo "Peak Hour," . $analytics['peak_hour'] . ":00\r\n";
            echo "Average Daily Visits," . $analytics['avg_daily_visits'] . "\r\n";
            echo "Monthly Treatments," . $analytics['monthly_treatments'] . "\r\n";
            echo "Average Service Time (Minutes)," . $analytics['avg_service_time'] . "\r\n";
            ...
            if ($exportFormat === 'excel') {
                foreach ($visitorGroups as $group => $visitors) {
                    ...
                    if ($groupType === 'student') {
                        echo '"' . str_replace('"', '""', 'Student Course: ' . $groupName) . '"\r\n';
                    } else {
                        echo '"Teacher Visits"\r\n';
                    }
                    echo "Name,Type,Course / Department,Reason,Date,Time\r\n";
                    foreach ($visitors as $visitor) {
                        ...
                        echo '"' . str_replace('"', '""', $visitor['name'] ?? '-') . '",';
                        echo '"' . str_replace('"', '""', ucfirst($visitor['visitor_type'] ?? 'student')) . '",';
                        echo '"' . str_replace('"', '""', $visitor['course_department'] ?? '-') . '",';
                        echo '"' . str_replace('"', '""', $visitor['reason_for_visit'] ?? '-') . '",';
                        echo '"' . str_replace('"', '""', $dateValue) . '",';
                        echo '"' . str_replace('"', '""', $clockValue) . '"\r\n';
                    }
                    echo "\r\n";
                }
            } else {
                echo "<h2>Condition Visitors Export</h2>";
                echo "<p>Filtered by: ".htmlspecialchars($exportTimeframe)." / ".htmlspecialchars($exportReason ?: 'All Reasons')." / ".htmlspecialchars($exportCourse ?: 'All Courses')." / ".htmlspecialchars($exportType ?: 'All Types')."</p>";

                foreach ($visitorGroups as $group => $visitors) {
                    list($groupType, $groupName) = explode(':', $group, 2);
                    if ($groupType === 'student') {
                        echo "<h3>Student - Course: " . htmlspecialchars($groupName) . "</h3>";
                    } else {
                        echo "<h3>Teacher Visits</h3>";
                    }
                    echo "<table border=1 cellpadding=5 cellspacing=0 style='border-collapse:collapse; width:100%; margin-bottom:20px;'>";
                    echo "<thead><tr><th>Name</th><th>Type</th><th>Course / Department</th><th>Reason</th><th>Time In</th></tr></thead><tbody>";
                    foreach ($visitors as $visitor) {
                        echo "<tr>";
                        echo "<td>" . htmlspecialchars($visitor['name'] ?? '-') . "</td>";
                        echo "<td>" . htmlspecialchars(ucfirst($visitor['visitor_type'] ?? 'student')) . "</td>";
                        echo "<td>" . htmlspecialchars($visitor['course_department'] ?? '-') . "</td>";
                        echo "<td>" . htmlspecialchars($visitor['reason_for_visit'] ?? '-') . "</td>";
                        echo "<td>" . htmlspecialchars($visitor['time_in'] ?? '-') . "</td>";
                        echo "</tr>";
                    }
                    echo "</tbody></table>";
            ...
            echo "Need Category,Cases,Percentage\r\n";
            if (!empty($analytics['top_needs'])) {
                foreach ($analytics['top_needs'] as $need) {
                    echo '"' . str_replace('"', '""', $need['need']) . '",' . $need['count'] . ',' . $need['percentage'] . "%\r\n";
                }
            }
            ...
            echo "Medicine Name,Stock Quantity,Min Stock Level,Expiry Date\r\n";
            foreach ($medicines as $med) {
                echo '"' . str_replace('"', '""', $med['name']) . '",' . $med['stock_quantity'] . ',' . $med['min_stock_level'] . ',"' . ($med['expiry_date'] ?: 'N/A') . "\"\r\n";
            }
            ...
            echo "ID,Student Name,Reason,Status,Submitted At,Reviewed At\r\n";
            foreach ($clearances as $clearance) {
                echo $clearance['id'] . ',"' . str_replace('"', '""', $clearance['full_name']) . '","' . str_replace('"', '""', $clearance['reason']) . '",' . $clearance['status'] . ',"' . $clearance['submitted_at'] . '","' . ($clearance['reviewed_at'] ?: 'N/A') . "\"\r\n";
            }
        }
        ...
        echo "<html><head><title>Clinic " . ucfirst($reportType) . " Report</title></head><body>";
        echo "<h1>Clinic " . ucfirst($reportType) . " Report</h1>";
        echo "<p>Generated on: " . date('Y-m-d H:i:s') . "</p><br>";

        if ($reportType === 'overview') {
            echo "<h2>Analytics Overview</h2>";
            echo "<table border='1'><tr><th>Metric</th><th>Value</th></tr>";
            echo "<tr><td>Total Patients</td><td>" . $analytics['total_patients'] . "</td></tr>";
            echo "<tr><td>Monthly Visits</td><td>" . $analytics['monthly_visits'] . "</td></tr>";
            echo "<tr><td>Today's Visits</td><td>" . $analytics['today_visits'] . "</td></tr>";
            echo "<tr><td>Weekly Visits</td><td>" . $analytics['week_visits'] . "</td></tr>";
            echo "<tr><td>Average Rating</td><td>" . $analytics['avg_rating'] . "</td></tr>";
            echo "<tr><td>Total Medicines</td><td>" . $analytics['total_medicines'] . "</td></tr>";
            echo "<tr><td>Low Stock Alerts</td><td>" . $analytics['low_stock_alerts'] . "</td></tr>";
            echo "<tr><td>Expired Medicines</td><td>" . $analytics['expired_medicines'] . "</td></tr>";
            echo "<tr><td>Pending Clearances</td><td>" . $analytics['pending_clearances'] . "</td></tr>";
            echo "<tr><td>Approved Clearances (Monthly)</td><td>" . $analytics['approved_clearances'] . "</td></tr>";
            echo "<tr><td>Rejected Clearances (Monthly)</td><td>" . $analytics['rejected_clearances'] . "</td></tr>";
            echo "<tr><td>Peak Hour</td><td>" . $analytics['peak_hour'] . ":00</td></tr>";
            echo "<tr><td>Average Daily Visits</td><td>" . $analytics['avg_daily_visits'] . "</td></tr>";
            echo "<tr><td>Monthly Treatments</td><td>" . $analytics['monthly_treatments'] . "</td></tr>";
            echo "<tr><td>Average Service Time (Minutes)</td><td>" . $analytics['avg_service_time'] . "</td></tr>";
            echo "</table>";
        } elseif ($reportType === 'visits') {
            $exportTimeframe = $_POST['cond_timeframe'] ?? $_GET['cond_timeframe'] ?? 'month';
            $exportReason = trim($_POST['cond_reason'] ?? $_GET['cond_reason'] ?? '');
            ...
            echo "<h2>Condition Visitors Report</h2>";
            echo "<p>Filtered by: " . htmlspecialchars($exportTimeframe) . " / " . htmlspecialchars($exportReason ?: 'All Reasons') . " / " . htmlspecialchars($exportCourse ?: 'All Courses') . " / " . htmlspecialchars($exportType ?: 'All Types') . "</p>";
            ...
            foreach ($visitorGroups as $group => $visitors) {
                ...
                echo "<table border='1'><tr><th>Name</th><th>Type</th><th>Course / Department</th><th>Reason</th><th>Date</th><th>Time</th></tr>";
                foreach ($visitors as $visitor) {
                    $timeValue = $visitor['time_in'] ?? '';
                    $dateValue = $timeValue ? date('Y-m-d', strtotime($timeValue)) : '-';
                    $clockValue = $timeValue ? date('H:i:s', strtotime($timeValue)) : '-';
                    echo "<tr><td>" . htmlspecialchars($visitor['name'] ?? '-') . "</td><td>" . htmlspecialchars(ucfirst($visitor['visitor_type'] ?? 'student')) . "</td><td>" . htmlspecialchars($visitor['course_department'] ?? '-') . "</td><td>" . htmlspecialchars($visitor['reason_for_visit'] ?? '-') . "</td><td>" . htmlspecialchars($dateValue) . "</td><td>" . htmlspecialchars($clockValue) . "</td></tr>";
                }
                echo "</table>";
                echo "<div style='page-break-after: always;'></div>";
            }
        } elseif ($reportType === 'needs') {
            echo "<h2>Visit Needs Report</h2>";
            echo "<table border='1'><tr><th>Need Category</th><th>Cases</th><th>Percentage</th></tr>";
            if (!empty($analytics['top_needs'])) {
                foreach ($analytics['top_needs'] as $need) {
                    echo "<tr><td>" . htmlspecialchars($need['need']) . "</td><td>" . $need['count'] . "</td><td>" . $need['percentage'] . "%</td></tr>";
                }
            }
            echo "</table>";
        } elseif ($reportType === 'inventory') {
            ...
            echo "<table border='1'><tr><th>Medicine Name</th><th>Stock Quantity</th><th>Min Stock Level</th><th>Expiry Date</th></tr>";
            foreach ($medicines as $med) {
                echo "<tr><td>" . htmlspecialchars($med['name']) . "</td><td>" . $med['stock_quantity'] . "</td><td>" . $med['min_stock_level'] . "</td><td>" . ($med['expiry_date'] ?: 'N/A') . "</td></tr>";
            }
            echo "</table>";
        } elseif ($reportType === 'clearances') {
            $pdo = get_db();
            $clearances = $pdo->query("SELECT cr.id, u.full_name, cr.reason, cr.status, cr.submitted_at, cr.reviewed_at FROM clinic_clearance_requests cr JOIN users u ON cr.user_id = u.id ORDER BY cr.submitted_at DESC")->fetchAll(PDO::FETCH_ASSOC);
            echo "<h2>Clearance Requests Report</h2>";
            echo "<table border='1'><tr><th>ID</th><th>Student Name</th><th>Reason</th><th>Status</th><th>Submitted At</th><th>Reviewed At</th></tr>";
            foreach ($clearances as $clearance) {
                echo "<tr><td>" . $clearance['id'] . "</td><td>" . htmlspecialchars($clearance['full_name']) . "</td><td>" . htmlspecialchars($clearance['reason']) . "</td><td>" . $clearance['status'] . "</td><td>" . $clearance['submitted_at'] . "</td><td>" . ($clearance['reviewed_at'] ?: 'N/A') . "</td></tr>";
            }
            echo "</table>";
        }
        echo "</body></html>";
